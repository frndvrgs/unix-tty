...
```console
$ x64rag retrieval --pretty status
...
```console
$ x64rag retrieval ingest scifi-library/book-01-sands-of-araxis.md -k araxis --source-type book
{
...
$ x64rag retrieval ingest scifi-library/book-02-the-chronodust-war.md -k araxis --source-type book
{
...
for f in scifi-library/book-*.md; do
  x64rag retrieval ingest "$f" -k araxis --source-type book
done
```
...
```console
$ x64rag retrieval ingest --text "The Araxeen calendar divides the year into 13 months of 28 days each, named after the 13 known sandweaver migration routes. The first month, Deepcurrent, marks the beginning of the breeding season when juvenile weavers emerge from the polar caves." -k araxis --source-type notes
{
  "source_id": "b7e41f93-...",
  "chunk_count": 1,
  "status": "completed"
...
`x64rag retrieval retrieve` returns source chunks without LLM generation. With reranking enabled, results are re-scored by a cross-encoder — the top result scores 0.79 (vs ~0.52 without reranking):

```console
$ x64rag retrieval retrieve "What is chronodust and where does it come from?" -k araxis --min-score 0.4
{
  "chunks": [
    {
...
```console
$ x64rag retrieval --json retrieve "sandweaver biology" -k araxis | jq '.chunks[0].content'
...
```console
$ x64rag retrieval retrieve "that thing about the caves glowing" -k araxis --min-score 0.3
{
  "chunks": [
    {
...
`x64rag retrieval query` retrieves relevant chunks and generates a synthesized answer via LLM. With grounding gates enabled, the answer shows grounding confidence:

```console
$ x64rag retrieval --pretty query "Describe the three pillars of the Cognitive Order and how they compare to the Araxeen Resonance practice" -k araxis
...
```console
$ x64rag retrieval --pretty query "What happened to Dhamira throughout the saga?" \
...
```console
$ x64rag retrieval --pretty query "What was Dhamira's relationship with the Resonance Academy?" \
    -k araxis --session araxis-research
...
```console
$ x64rag retrieval session list
  araxis-research (4 turns)

$ x64rag retrieval session clear araxis-research
...
```console
$ x64rag retrieval query "What is the name of the planet where the sandweavers originated?" -k araxis
{
...
The real power of the x64rag CLI for writing is the feedback loop: ingest a draft, check it against the existing canon, revise, re-ingest.
...
```console
$ x64rag retrieval ingest scifi-library/draft-book-09-chapter-01.md -k araxis --source-type draft
{
...
```console
$ x64rag retrieval --pretty query "The draft has Dhamira using Projection. Is that consistent \
...
The x64rag system cross-references the draft against all 8 published books and catches the inconsistency. After revising the draft, remove the old version and re-ingest:

```console
$ x64rag retrieval knowledge remove 54793450-bef0-430a-ac37-6a7c42bbd6c2
$ x64rag retrieval ingest scifi-library/draft-book-09-chapter-01.md -k araxis --source-type draft
```
...
```console
$ x64rag retrieval --pretty query "I'm writing book 9. What was the state of the world \
...
```console
$ x64rag retrieval --pretty query "What is the relationship between Kael and Dhamira throughout the saga?" -k araxis
...
```console
$ x64rag retrieval retrieve "stillsuit water discipline araxeen survival" -k araxis
{
  "chunks": [
    {
...
```console
$ x64rag retrieval --pretty knowledge list -k araxis
...
```console
$ x64rag retrieval knowledge get a76d8591-a640-4180-a27c-c7c98585716f
{
  "source_id": "a76d8591-a640-4180-a27c-c7c98585716f",
  "status": "completed",
  "chunk_count": 12,
...
```console
$ x64rag retrieval --pretty knowledge chunks a76d8591-a640-4180-a27c-c7c98585716f
...
```console
$ x64rag retrieval knowledge remove a76d8591-a640-4180-a27c-c7c98585716f
{
  "source_id": "a76d8591-a640-4180-a27c-c7c98585716f",
  "deleted_vectors": 12
}
...
1. **Research lore**: `x64rag retrieval retrieve "sandweaver biology" -k araxis --min-score 0.4`
2. **Multi-turn research**: `x64rag retrieval query "What happened to Dhamira?" -k araxis --session book5-rewrite`
3. **Follow up**: `x64rag retrieval query "What was her role in the moratorium?" -k araxis --session book5-rewrite`
4. **Ask synthesized questions**: `x64rag retrieval query "What happened to Navigator Primus Leth?" -k araxis`
5. **Verify consistency**: `x64rag retrieval retrieve "Kael's age timeline" -k araxis`
6. **Add notes**: `x64rag retrieval ingest --text "New timeline note: ..." -k araxis --source-type notes`
7. **Manage sessions**: `x64rag retrieval session list` / `x64rag retrieval session clear book5-rewrite`
8. **Manage sources**: `x64rag retrieval knowledge list -k araxis` / `x64rag retrieval knowledge remove <id>`
